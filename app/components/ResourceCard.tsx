...
import { MapPin, Clock, Star, Heart, Edit, Trash2, Image as ImageIcon, Mail } from 'lucide-react';
...
export function ResourceCard({ resource, onDelete, onEdit, onSelect, onFavoriteChange }: ResourceCardProps) {
    ...
    const [isLiked, setIsLiked] = useState(!!resource.isFavorited);
    ...
    const handleLike = async (e: React.MouseEvent) => {
        ...
        try {
          ...
          const response = await fetch(
            `/api/resources/favorite?id=${resource.id}&action=${action}`,
            { method: "POST" }
          );
